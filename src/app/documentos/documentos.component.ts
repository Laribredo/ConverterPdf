...
export class DocumentosComponent implements OnInit {
  ...
  ngOnInit(): void {
    let data = new Date();
    this.emissao = data.getDate() + "/" +( (data.getMonth()+1) < 10 ? "0" + (data.getMonth()+1) : data.getMonth() ) + "/" + data.getFullYear();
  }
  ...
  preencheCamposForm(){
   ...
   pdfConverter<Promise>(pdfA:any){
    ...
    fileReader.onload = function() {
        ...
        function requestPDFJS(typedarray)
        {
